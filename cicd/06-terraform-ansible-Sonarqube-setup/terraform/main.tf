...
resource "aws_instance" "my_ec2_instance" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ubuntu -i ${aws_instance.my_ec2_instance.public_ip}, --private-key ../keys/mykey -e 'pub_key=../keys/mykey.pub' ../ansible/sonarqube-playbook.yaml
      EOT
  }
